...
const toggleQuote = () => {
    const container = document.getElementsByClassName('quote__container')[0];
    container.style.display = container.style.display === 'block' ? 'none' : 'block';
};
const Edit_Options = (text) => {
    var edit_menu = document.getElementById("edit-dropdown");
    edit_menu.style.display = 'none';
    // alert(text);
    const container = document.getElementsByClassName('edit__New__container')[0];
    container.style.display = container.style.display === 'block' ? 'none' : 'block';
}
...
const change_type = (type) => {
    ...
    // Access the active type from the container
    const container = document.getElementsByClassName('type__container')[0];
    const activeType = container.getAttribute('type');
   
    if (type === '3D') {
        if (!activeType || activeType === '3D') {
            alert('Already on 3D plan!');
        } else {
            // Close the currently active view (Floor or Plan)
            container.style.display = 'none'; // Hide the container
            container.setAttribute('type', '3D');
        }
        return;
    }

    if (type === 'Floor' || type === 'Plan') {
        container.style.display = 'flex';
        container.setAttribute('type', type);
        container.style.marginTop = type === 'Floor' ? '28px' : '15px'; 
    }

    // Hide the dropdown menu after selection
    const type_menu = document.getElementById("type-dropdown");
    type_menu.style.display = 'none';
};

const change_Cam_Angle = (angle) => {
    console.log("Angle: ",angle);
    var camera_angle = document.getElementById("cam-dropdown");
    localStorage.setItem("Angle", angle)
    // camera_angle.style.display = camera_angle.style.display === "none" ? "block" : "none";
}
const slab_property = (setting) => {
    console.log("slab: ",setting);
    var camera_angle = document.getElementById("slab-dropdown");
    localStorage.setItem("slab", setting)
    // camera_angle.style.display = camera_angle.style.display === "none" ? "block" : "none";
}
function toggleDropdown() {
    var dropdownMenu = document.getElementById("views-dropdown");
    dropdownMenu.style.display = dropdownMenu.style.display === "none" ? "block" : "none";
}

function selectOption(option) {
    var checkboxes = document.querySelectorAll('.views-input');
...
function change_view() {
    var container = document.getElementsByClassName('view__container')[0];
    container.style.display = container.style.display === 'block' ? 'none' : 'block';   
}