...
import { faTimesCircle, faDoorOpen } from '@fortawesome/free-solid-svg-icons';  // Use an appropriate icon
import './metalSlidingDoor.css'; // Ensure you create and import the CSS file

var loaded_meshes = [];
var id = 0;

export const meshLoaderMetalSlidingDoor = (loadedMeshes) => {
  loaded_meshes = loadedMeshes;
  return loaded_meshes;
}

const MetalSlidingDoorMenu = ({ itemClass }) => {
  const box = useRef(null);
  const [width, setWidth] = useState(5);
  const [height, setHeight] = useState(1);
  const [position, setPosition] = useState(6);
  const [doors, set_doors] = useState(false);
  ...
  return (
    <div className={`metal-sliding-door-container ${itemClass}`}>
      {doors === true ? <>
        <span className="metal-sliding-door-headings">
          <p>Metal Sliding Doors</p>
          <FontAwesomeIcon icon={faTimesCircle} onClick={() => set_doors(!doors)} />
        </span>
        ...
        : <>
          <button onClick={() => set_doors(!doors)} className='metal-sliding-door'>
            <FontAwesomeIcon icon={faDoorOpen} style={{ fontSize: '5rem' }} />
             Metal Sliding Door
          </button>
        </>
      }
...
export default MetalSlidingDoorMenu;
