...
import { faTimesCircle, faWindowRestore } from '@fortawesome/free-solid-svg-icons'; // Use an appropriate icon
import './barnWindow.css'; // Ensure you create and import the CSS file

var loaded_meshes = [];
var id = 0;

export const meshLoaderBarnWindow = (loadedMeshes) => {
  loaded_meshes = loadedMeshes;
  return loaded_meshes;
}

const BarnWindowMenu = ({ itemClass }) => {
  const box = useRef(null);
  const [width, setWidth] = useState(5);
  const [height, setHeight] = useState(1);
  const [position, setPosition] = useState(6);
  const [windows, set_windows] = useState(false);
  ...
  return (
    <div className={`barn-window-container ${itemClass}`}>
      {windows === true ? <>
        <span className="barn-window-headings">
          <p>Barn Windows</p>
          <FontAwesomeIcon icon={faTimesCircle} onClick={() => set_windows(!windows)} />
        </span>
        ...
        : <>
          <button onClick={() => set_windows(!windows)} className='barn-window'>
            <FontAwesomeIcon icon={faWindowRestore} style={{ fontSize: '5rem' }} />
              barnwindow
          </button>
        </>
      }
