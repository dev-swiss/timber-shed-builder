...
import { faTimesCircle, faThLarge } from '@fortawesome/free-solid-svg-icons'; // Use an appropriate icon
import './ConcreteWall.css'; // Ensure you create and import the CSS file

var loaded_meshes = [];
var id = 0;

export const meshLoaderConcreteWall = (loadedMeshes) => {
  loaded_meshes = loadedMeshes;
  return loaded_meshes;
}

const ConcreteWallMenu = ({ itemClass }) => {
        ...
        pointerMoveTimeout = setTimeout(() => {
          if (performance.now() - lastEventTime >= delay) {
            ...
            if (result.hit && isValidMeshForBox(result.pickedMesh)) {
              ...
              hl.current.addMesh(result.pickedMesh, Color3.Red());
            }
          }
        }, delay);
      };
  ...
  return (
    <div className={`concrete-wall-container ${itemClass}`}>
      {walls === true ? <>
        <span className="concrete-wall-headings">
          <p>Concrete Wall</p>
          <FontAwesomeIcon icon={faTimesCircle} onClick={() => setWalls(!walls)} />
        </span>
        ...
        : <>
          <button onClick={() => setWalls(!walls)} className='concrete-wall'>
            <FontAwesomeIcon icon={faThLarge} style={{ fontSize: '5rem' }} />
            Concrete Wall
          </button>
        </>
      }
...
export default ConcreteWallMenu;
