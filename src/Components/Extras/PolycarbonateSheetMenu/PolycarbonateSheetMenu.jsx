...
import { faTimesCircle, faLayerGroup } from '@fortawesome/free-solid-svg-icons'; // Use an appropriate icon
import './PolycarbonateSheetMenu.css'; // Ensure you create and import the CSS file

var loaded_meshes = [];
var id = 0;

export const meshLoaderPolycarbonateSheet = (loadedMeshes) => {
  loaded_meshes = loadedMeshes;
  return loaded_meshes;
}

const PolycarbonateSheetMenu = ({ itemClass }) => {
  const box = useRef(null);
  const [width, setWidth] = useState(5);
  const [height, setHeight] = useState(1);
  const [position, setPosition] = useState(6);
  const [sheets, set_sheets] = useState(false);
  ...
  return (
    <div className={`polycarbonate-sheet-container ${itemClass}`}>
      {sheets === true ? <>
        <span className="polycarbonate-sheet-headings">
          <p>Polycarbonate Sheets</p>
          <FontAwesomeIcon icon={faTimesCircle} onClick={() => set_sheets(!sheets)} />
        </span>
...
<input
        ...
        : <>
          <button onClick={() => set_sheets(!sheets)} className='polycarbonate-sheet'>
            <FontAwesomeIcon icon={faLayerGroup} style={{ fontSize: '5rem' }} />
            Polycarbonate Sheet
          </button>
        </>
      }
...
export default PolycarbonateSheetMenu;
